...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Combat|Input", meta = (DisplayName = "切换攻击方向最小间隔"))
	...
	/** 获取攻击触发计数器，AnimBP 可用它判断是否触发了新的攻击。 */
	UFUNCTION(BlueprintPure, Category = "Combat", meta = (DisplayName = "获取攻击触发计数"))
	int32 GetAttackTriggerCounter() const { return AttackTriggerCounter; }

	/** 当前是否存在正在播放或尚未结束的攻击。 */
	UFUNCTION(BlueprintPure, Category = "Combat", meta = (DisplayName = "当前攻击是否激活"))
